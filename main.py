import os, re, datetime, textwrap, uuid, traceback, time
import requests
...
BASES = [
    "https://news.nate.com/rank",                 # 데스크톱
    "https://m.news.nate.com/rank/interest",      # 모바일(백업)
]
W, H = 1080, 1920  # Shorts(세로)
OUT_DIR = "out"
ASSETS = "assets"
...
def ensure_font():
    for p in FONT_PATH_CANDIDATES:
        if os.path.exists(p):
            return p
    raise RuntimeError("❌ 한글 폰트를 찾지 못했습니다. assets/fonts에 NanumGothic.ttf 추가하거나 워크플로 설치를 확인하세요.")
...
def fetch_html(url):
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
                      "(KHTML, like Gecko) Chrome/124.0 Safari/537.36 DalsuBot/1.2"
    }
...
def parse_from_rank(url, take=6):
    html = fetch_html(url)
    # 디버그 저장
    open(os.path.join(OUT_DIR, "debug_rank.html"), "w", encoding="utf-8").write(html)
...
def fetch_summary(article_url, max_len=30):
    try:
        ...
        soup = BeautifulSoup(html, "lxml")
        p_tags = soup.select("div.article p") or soup.find_all("p")
...
def make_slide(img_path, title, dur, caption=None):
    ...
    if title:
        draw.multiline_text((60, 80), wrap(title, 18), fill="white",
                            font=font_title, stroke_fill="black", stroke_width=2)
    if caption:
        draw.multiline_text((60, H-400), wrap(caption, 20), fill="white",
                            font=font_body, stroke_fill="black", stroke_width=2)
...
def run():
    ...
    # 1) 랭킹 페이지 시도 (데스크톱 → 모바일 백업)
    items = []
    errors = []
    for base in BASES:
        try:
            items = parse_from_rank(base, TAKE)
            if items: break
    ...
    if not items:
        raise RuntimeError("랭킹에서 뉴스를 찾지 못했습니다. " + " | ".join(errors))
...
if __name__ == "__main__":
    try:
        ...
        # 실패해도 디버깅 자료를 남기고 종료
        ensure_dirs()
        open(os.path.join(OUT_DIR, "error.txt"), "w", encoding="utf-8").write(
            f"{e}\n\nTRACEBACK:\n{traceback.format_exc()}"
        )
        # 인트로/클로징만 담은 짧은 대체 영상 생성(업로드 확인용)
        try:
            intro = ImageClip(os.path.join(ASSETS, "intro.png")).set_duration(3)
            outro = ImageClip(os.path.join(ASSETS, "outro.png")).set_duration(3)
            concatenate_videoclips([intro, outro], method="compose").write_videofile(
                os.path.join(OUT_DIR, "fallback.mp4"), fps=30, codec="libx264", audio=False
            )
        except:
            pass
        # 여기서 예외를 다시 던지지 않음 → 업로드 단계까지 진행
